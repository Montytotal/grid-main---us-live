...
Set up a cron job to execute the `update.php` script (using the [PHP CLI SAPI](https://www.php.net/manual/en/features.commandline.usage.php)) every 15 minutes. The cron job must run as a user with write access to `public/favicon.svg`, `public/index.html`, and `public/us/index.html`.
...
The national US page uses EIA as the base national fuel-mix snapshot.

Future regional pages should use direct official ISO, RTO, or BPA data where a
regional source exists. The intended direct-source set is CAISO, ERCOT, PJM,
MISO, NYISO, ISO-NE, SPP, and BPA. Regions without a direct source should fall
back to EIA-930 balancing-authority data.

A later blended national model may combine direct ISO/RTO/BPA data where
available with EIA fallback coverage elsewhere, but the current national page
does not do that yet.

PHP classes: [UsLatestGeneration](classes/Data/UsLatestGeneration.php),
[EiaFuelMix](classes/Data/EiaFuelMix.php)
